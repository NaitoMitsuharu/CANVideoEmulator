; Inno Setup script for CAN Vehicle Replay Player.
;
; Produces installer/CanReplayPlayer-Setup-<version>-win-x64.exe from the
...
#define AppName "CAN Vehicle Replay"
#define AppExeName "CanReplayPlayer.exe"
#define AppPublisher "CAN Vehicle Replay"
...
OutputDir={#OutputDir}
OutputBaseFilename=CanReplayPlayer-Setup-{#AppVersion}-win-x64
...
begin
  ...
  if not PcanBasicInstalled then
    MsgBox('The PEAK-System PCAN-USB driver was not detected on this computer.' + #13#10#13#10 +
           'CAN Vehicle Replay will still start and play scenarios in Demo Mode, ' +
  ...
  if not ScenariosInstalled then
    MsgBox('No scenarios were installed.' + #13#10#13#10 +
           ...
           'recorded driving data, start CAN Vehicle Replay and use the ' +
