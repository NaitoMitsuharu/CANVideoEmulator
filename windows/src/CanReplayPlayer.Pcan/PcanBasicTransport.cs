...
public sealed class PcanBasicTransport : ICanTransport
{
    ...
    public CanSendResult Send(in CanFrame frame)
    {
        ...
        if (status is PcanStatus.TransmitQueueFull or PcanStatus.TransmitBufferFull)
        {
            return CanSendResult.QueueFull(detail);
        }

        if (status is PcanStatus.BusOff or PcanStatus.BusHeavy or PcanStatus.BusPassive
            or PcanStatus.BusWarning or PcanStatus.BusLight)
        {
            lock (_gate) { _status = MapStatus(status, initialising: false); }
        }
    ...
    /// <summary>
    /// Read hardware status without transmitting -- what TEST CONNECTION uses
    /// (requirement 23).
    /// </summary>
    ...
    /// <summary>
    /// Clear the controller's queues and error state after a BUS OFF, without
    /// transmitting anything.
    /// </summary>
    public CanTransportStatus Reset()
    {
        lock (_gate)
        {
            if (!_open)
    ...
    internal static CanTransportStatus MapStatus(PcanStatus status, bool initialising)
    {
        ...
        var health = status switch
        {
            ...
            PcanStatus.BusLight => CanTransportHealth.BusLight,
            PcanStatus.BusHeavy or PcanStatus.BusWarning => CanTransportHealth.BusHeavy,
            PcanStatus.BusOff or PcanStatus.BusPassive => CanTransportHealth.BusOff,
            ...
            PcanStatus.IllegalHardwareHandle or PcanStatus.IllegalHandle
                or PcanStatus.IllegalNetHandle or PcanStatus.IllegalClientHandle =>
                CanTransportHealth.NotConnected,
            PcanStatus.Initialize when initialising => CanTransportHealth.NotConnected,
        ...
        if (health == CanTransportHealth.ChannelInUse)
        {
            detail = $"{detail} -- another application (often PCAN-View) already " +
                     "has this channel open. Close it and press Refresh.";
        }
