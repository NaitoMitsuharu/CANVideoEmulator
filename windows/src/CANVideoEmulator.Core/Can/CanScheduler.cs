...
public sealed class CanScheduler : IAsyncDisposable
{
    ...
    private static int Severity(CanTransportHealth health) => health switch
    {
        ...
        CanTransportHealth.BusPassive => 6,
                     ...
                     TimeSpan startPosition, CancellationToken token)
    {
        ...
        var initialStatus = transport.RefreshStatus();
        NoteHealth(initialStatus);
        if (!initialStatus.IsUsable) return;
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            else
            {
                ...
                var failedStatus = transport.RefreshStatus();
                NoteHealth(failedStatus);
                if (!failedStatus.IsUsable) return;
            }
            ...
            if (sentAt - lastHealthCheck >= HealthCheckInterval)
            {
                ...
                var currentStatus = transport.RefreshStatus();
                NoteHealth(currentStatus);
                if (!currentStatus.IsUsable) return;
            }
