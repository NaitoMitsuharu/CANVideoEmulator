...
/// (<c>%APPDATA%\CANVideoEmulator\config.json</c>), which is writable without
...
public sealed class AppSettings
{
    ...
    [JsonPropertyName("dataset_download_directories")]
    public List<string> DatasetDownloadDirectories { get; set; } = [];
    ...
    public static string DefaultScenarioDirectory
    {
        get
        {
            var parent = Directory.GetParent(AppContext.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar))?.FullName;
            return parent is not null && (Directory.Exists(Path.Combine(parent, "Scenarios")) ||
                Directory.Exists(Path.Combine(parent, ".git")))
                ? Path.Combine(parent, "Scenarios")
    ...
    public static AppSettings Load()
    {
        try
        {
            var source = ConfigPath;
            if (!IsPortable && !File.Exists(source))
                source = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                    "CanReplayPlayer", "config.json"); // migrate the previous product's settings
            ...
            var settings = JsonSerializer.Deserialize<AppSettings>(
                File.ReadAllText(source), Options);
