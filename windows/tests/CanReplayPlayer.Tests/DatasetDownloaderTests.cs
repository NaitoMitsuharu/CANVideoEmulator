...
{
    [Fact]
    public void OnlyTheRav4ChunksAreOffered()
    {
        // Chunks 3-10 are a Honda Civic, and this build ships no Civic profile,
        // so downloading one would produce packages that cannot be decoded.
        Assert.Equal(2, DatasetCatalog.Chunks.Count);
        Assert.All(DatasetCatalog.Chunks, c => Assert.Equal("Toyota RAV4", c.Vehicle));
    }
    ...
    public void LookupIsCaseInsensitiveAndMissesReturnNull()
    {
        ...
        Assert.Null(DatasetCatalog.Find("chunk_7"));
    }
}
