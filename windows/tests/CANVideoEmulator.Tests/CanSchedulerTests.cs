...
public class CanSchedulerTests
{
    ...
    [Fact]
    public void ClosedTransportReportsFaultBeforeSending()
    {
        ...
        scheduler.Start(clock.CurrentTime);
        clock.Play();
        Assert.True(WaitUntil(() => scheduler.WorstHealth == CanTransportHealth.NotConnected));
        scheduler.Stop();

        Assert.Equal(0, scheduler.Snapshot().FramesSent);
        Assert.Equal(0, scheduler.Snapshot().FramesScheduled);
    }
}
